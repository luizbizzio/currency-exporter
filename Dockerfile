...
ENV CURRENCY_EXPORTER_CONFIG=/config/config.yaml
...
COPY currency_exporter.py /app/currency_exporter.py

EXPOSE 7575

HEALTHCHECK --interval=30s --timeout=5s --start-period=15s --retries=3 \
  CMD python -c "import os,sys,yaml,urllib.request; p=7575; path=os.environ.get('CURRENCY_EXPORTER_CONFIG','/config/config.yaml'); \
  ...
  sys.exit(0 if 'currency_exporter_up' in d else 1)"

USER app

ENTRYPOINT ["python", "/app/currency_exporter.py"]
